...
\subsection{5. Qualité et Déploiement (DevOps)}
\begin{itemize}
    \item \textbf{Tests Backend (Jest + Supertest)} : Couverture des algorithmes de pénalité (Unitaires) et des endpoints API (Intégration).
    \item \textbf{Tests Frontend (Vitest)} : Validation du rendu des composants React et de la navigation.
    \item \textbf{Déploiement Cloud (CI/CD)} :
    \begin{itemize}
        \item Backend déployé sur \textbf{Railway} (Node.js/Express + SQLite).
        \item Frontend déployé sur \textbf{Vercel} (React/Vite).
        \item Pipeline de déploiement automatisé via script \texttt{deploy.sh}.
    \end{itemize}
    \item \textbf{Documentation} : Rédaction du \texttt{USER\_GUIDE.md} et du Guide de Déploiement Cloud.
\end{itemize}
...
\begin{itemize}
    ...
    \item Mise en place réussie d'une stratégie de tests robuste (Unitaires \& Intégration) en fin de sprint.
\end{itemize}
